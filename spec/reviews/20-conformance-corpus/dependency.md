...
cases are about (FND-520). The contract publishes a diagnostic *pattern* but no
...
touching `schema/` or `docs/` to record them (FND-521, FND-522). And FR-038-CON-2
...
into the very ticket the parallelism exists to stay independent of (FND-523).
Inside the slice the frontmatter graph carries one cycle: FR-035's own gates are
decided by the FR-036 oracle, while FR-036 declares FR-035 upstream (FND-519).
...
(FND-528, FND-529). Downstream, #11 and quire-contract-ir#52 are named as
consumers by requirements that do not state what they consume (FND-524,
FND-531), and #36 — the second IR producer — appears in neither the adapter
...
graph TD
  ...
  CODES[diagnostic code registry - FND-521]
  DSHAPE[adapter diagnostic shape reconciliation - FND-522]
  MULTIDOC[multi-document case slot - FND-520]
  AC19[issue 19 acceptance criteria - FND-523]
  DEFECTS[recorded prototype-emitter divergences - FND-528]
...
The `FR-036 --> FR-035` edge is the cycle of FND-519; it disappears once the
...
the missing frontmatter edge of FND-532: FR-038-AC-6 and FR-038-AC-8 are
decided by the oracle, not by the schema.

## Logical Dependency Order

1. Settle the five unowned prerequisites: the diagnostic code registry and its
   owning contract ticket (FND-521), the reconciliation between
   `adapter-result.schema.json` and `common.schema.json#/$defs/diagnostic`
   (FND-522), the multi-document case slot for manifest, lock, mapping, and
   profile inputs (FND-520), the replacement of FR-038-CON-2's issue-text
   binding with merged contract clauses (FND-523), and the owner of the
   prototype-emitter divergence discovery (FND-528). The first three gate every
   step below.
2. FR-035 case, manifest, and adapter-result schemas, the base documents, and
   ...
   the FND-520 and FND-522 decisions.
3. FR-036 oracle: schema layer, cross-field layer, compatibility classification,
   ...
   FR-027..FR-029, the FND-521 code registry, and the rule families FR-036 does
   not yet enumerate (FND-526). The three oracle-decided FR-035 gates — TC-281,
...
4. FR-038 construct register and the case set — TC-308..317. Parallelizable per
   ...
   loss families wait on the FND-520 slot, and the defect family on FND-528.
...
6. FR-039 coverage account, mutation catalogue, thresholds, and import API —
   ...
   `./conformance` export lands unpublished pending #11 (FND-530).
7. NFR-015 verification — TC-325..328: repeat, locale-varied, and
   ...
   inspection whose scope FND-527 has to settle first.
8. NFR-016 verification — TC-329..332: changed-path gate, dependency
   ...
   any rebase onto #27's `package.json` and `Makefile` changes (FND-529).

## Cycles

One cycle at the requirement level, FR-035 → FR-036 → FR-035 (FND-519),
...
- filament-core-data#34 (IR v1.1, FR-027..FR-030) merged as 014bff7 and #35
  ...
  The #35 grammar is a declared but unconsumed dependency here (FND-525).
...
- filament-core-data#27 (promote the spike emitters into `src/`) is the head of
  ...
  `Makefile` edits (FND-529) and through the divergence set of FND-528.
- filament-core-data#19 (compiler core) is judged by this corpus and must not
  ...
  #19's acceptance criteria (FND-523); once that binds to merged contract
...
- filament-core-data#11 (publication) consumes the corpus as its gate and is
  the owner of the `./conformance` export's release (FND-530). What #11's own
  "shared golden corpus" criterion consumes is unstated (FND-524); publication
...
- filament-core-data#36 (spec-bundle extraction frontend) is a second producer
  ...
  FR-037 roster nor the FR-039 thresholds (FND-531). It is downstream of #19
...
- agent-ix/quire-contract-ir#52 and its #54 child consume the compiled domain
  ...
  import API is named but unspecified (FND-531). Neither is a prerequisite, and
