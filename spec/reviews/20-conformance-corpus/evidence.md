...
are the two the advisor is right about and are FND-790 and FND-791 below.
...
(FND-787), which is exactly the split SR-031 settled at FND-079 and SR-040
...
out separately (FND-797). Second, determinism is typed inconsistently: the same
"two runs, compare bytes" shape is `Property` at TC-290 and TC-326 and `Snapshot`
at TC-303 (FND-792). Third, two NFR metrics map to no test case, the shape SR-040
recorded at FND-168 (FND-789, FND-795, FND-796).
...
| Provenance: every expectation is quoted from a contract artifact and none is blessed from a run | TC-282, TC-286, TC-325, pending the `Unit` retype in FND-787 and the de-duplication in FND-803 |
| Case identity, id pattern, and 64-node minimization budget | TC-287, TC-288, pending FND-787 |
...
| Oracle verdict equals every case's authored `expected` block | Unit (TC-289); the non-object input case TC-296 pending FND-804 |
| Oracle determinism and locale independence | Property (TC-290) |
| Oracle terminates on cyclic graphs at the declared depth bound | Unit (TC-291); generated-cycle termination pending FND-799 |
| Five identity and resolution failures are five distinct diagnostic codes | Unit (TC-292) |
| Oracle imports no judged implementation and reads no forbidden path | Static (TC-293, TC-327); the NFR-015 metric trace pending FND-789 |
| `normalized` round-trip: a `1.0.0` document gains no bytes | TC-294, pending the `Property` retype in FND-793 |
...
| No adapter is compared with another adapter | Unit (TC-299) for the behaviour, Static (TC-300) for the source scan — one obligation, two kinds, per FND-788 |
...
| Harness report reproducibility | TC-303, pending the `Property` retype in FND-792 |
| Adapter process failure fails per case rather than skipping | Unit (TC-304) |
| Adapter roster and process isolation | TC-305 (pending FND-787), Static (TC-306) |
...
| Register sources and issue #19 criterion coverage | TC-314, TC-315, pending FND-787 |
...
| Promotion thresholds declared per backend with owning issues | TC-319, pending FND-787 |
| Mutation catalogue detection score and per-family coverage | TC-320, TC-321 — the `mutation-testing` measure of FND-805, TC-321 pending FND-787 |
| Import API works from an arbitrary working directory | Integration (TC-322); `Verification` cell pending FND-794 |
| Import API error behaviour and immutability | Unit (TC-323); immutability property pending FND-798 |
...
| Divergences are registered, not absorbed into expected results | Manual (TC-328), to be split into the FR-037 gate and a narrowed Inspection per FND-791 |
| Changed-path isolation and no new runtime dependency | Static (TC-329, TC-330); allow-list form pending FND-795, other-repository metric pending FND-796 |
| Conformance suites run offline from both toolchains | Integration (TC-331), pending the denied-network form in FND-800 |
| No package published and no consumer altered | Manual (TC-332), to be `Analysis` + `Static` per FND-790 |
