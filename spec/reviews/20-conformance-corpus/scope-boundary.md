...
| Package-, lock-, mapping-, and profile-level case expectations (FR-038 negatives) | Unallocated, see FND-827 | core |
| Compatibility classification for corpus `compatibility` cases | filament-core-data FR-025 contract, restated by the FR-036 oracle, see FND-828 | core |
| Consumer capability admission for a `required: true` extension | Consumer policy per ARCH-004, decided in the oracle, see FND-832 | external, decided here |
| `conformance/adapters/**` contents and each adapter's registry command | Unallocated, see FND-829 | external to this ticket |
| Adapter result emission conforming to `adapter-result.schema.json` | Issues #19, #21, #22, #23 — no reciprocal requirement, see FND-829 | external to this ticket |
| Divergence arbitration: corpus defect versus implementation defect | Unallocated, see FND-831 | cross-cutting |
| Promotion thresholds and the decision to promote a backend | Declared here, owned by issues #19, #21, #22, #23 and the issue #11 gate, see FND-830 | external to this ticket |
| `./conformance` export, `files` entry, and the published consumer surface | Issue #20 by FR-039, excluded by US-008 and `spec.md` 2.2, see FND-826 | external to this ticket |
| Corpus-versus-package version binding | Unallocated, see FND-833 | infrastructure |
| Python conformance gate contents (`tests/test_conformance_corpus.py`) | Issue #20, artifact unstated, see FND-834 | infrastructure |
...
| Issue #19 compiler frontend | Judged adapter | Assumed (FND-829) | FR-037-CON-1 registry row, `adapter-result.schema.json`, no obligation stated on #19 |
| Issue #21, #22, #23 backends | Judged adapters | Assumed (FND-829) | FR-037-CON-1 registry rows with `status: unavailable` and owning issue |
| Issue #4 prototype emitter | Judged implementation named by NFR-015 | Assumed (FND-837) | Defect register rows only; no registry adapter |
| Issue #27 emitter promotion | Concurrent change set on `src/` | Guaranteed | NFR-016 changed-path gate, NFR-016-AC-1 |
| FR-025 compatibility contract and `compatibility-report.schema.json` | Classification authority restated in the oracle | Assumed (FND-828) | FR-038 Inputs prose; no contract test between the two rule sets |
| `consumer-policy.schema.json` and declared consumer capabilities | Input to one oracle rule | Assumed (FND-832) | None named |
| Package manifests, locks, mappings, profiles (FR-021, FR-022) | Input to six required negative cases | Assumed (FND-827) | FR-038-AC-2 only; no oracle behavior states the rules |
| Issue #11 publication gate | Downstream consumer of coverage, thresholds, and the corpus | Assumed (FND-830, FND-826) | FR-039 Inputs prose, NFR-016 Dependencies |
| `package.json` consumer surface of `@agent-ix/filament-core-data` | Published export the corpus is added to | Assumed (FND-826, FND-833) | FR-039-AC-5 integration test; no surface-stability requirement |
| `make test` and `poetry run pytest` gates | Execution context for the corpus tests | Guaranteed | NFR-016-AC-3 offline run (artifact unstated, FND-834) |
...
other three edges. FND-826 and FND-833 are outward reach through the published
package that section 2.2 and US-008 explicitly disclaim. FND-827, FND-831, and
FND-832 are decisions the bundle requires but allocates to no component.
FND-828, FND-829, and FND-830 are authorities and interfaces asserted here over
work that other tickets own, without a contract test or a reciprocal
obligation on the far side. FND-834, FND-835, FND-836, and FND-837 are gate
