...
**So that** I can select TypeSpec or the documented JSON Schema fallback from retained compatibility evidence rather than ecosystem reputation.
...
### [US-004-EX-2] Capability failure selects the fallback

- **Given** a required P0 capability cannot be preserved or maintained at acceptable cost
- **When** the feasibility report is reviewed
- **Then** the report selects modular JSON Schema 2020-12, retains the failed evidence, and does not fall back to Avro or weaken the capability
...
| US-004-AC-2 | A failed P0 capability produces an evidence-backed no-go recommendation and the modular JSON Schema fallback without changing the current Avro authority or any consumer. | Demonstration (TC-128) |
...
All versions, commands, generated outputs, diagnostics, compatibility examples,
and maintenance judgments are retained. The spike is unpublished and writes only
inside its isolated directory.
...
decision. The principal risk is accepting an elegant authoring language whose
non-HTTP emitters require an unsustainable private compiler.

## Notes (Informative)

The report recommends a source and records the cost of being wrong. Human review,
not the spike script, promotes ADR-0004.
