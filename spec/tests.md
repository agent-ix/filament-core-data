...
| TC-1252 | slug("Config Version") is config-version, slug("A__B--C") is a-b-c, slug("--") is Unsluggable, and an enumeration value reading *** raises UNSLUGGABLE_NAME at its locus, blocking; a "---" title fails earlier as UNNAMEABLE_ARTIFACT | Unit | P1 | FR-095-AC-7 | 🚧 planned |
...
| TC-1292 | The rust-serde backend CLI over the lifted config-version-table document exits zero with zero diagnostics; generate --target typescript over the same document exits zero once issue #88 is fixed, refusing it with DUPLICATE_IDENTITY until then | Integration | P0 | FR-098-AC-8 | 🚧 TypeScript half blocked on issue #88 |
...
| TC-1304 | No module under src/ references SystemTime, Instant, std::env, env!, option_env!, a hostname API, an RNG, std::net, or Command; std::fs only in write.rs (reads: manifest.yaml per module root, the golden walk, inspect --ir; writes: the atomic outputs), every other module std::fs-free; the gate fails on a planted std::env::var in lower.rs | Static | P0 | NFR-031-AC-5 | 🚧 planned |
...
| TC-1306 | The crate suite run under unshare -rn (or an equivalent unprivileged network namespace) with cargo in offline mode passes, and the crate declares no dependency that opens a socket | Static | P1 | NFR-031-AC-7 | 🚧 planned |
