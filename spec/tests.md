...
| FR-094 | FR-094-AC-1..15, FR-094-CON-1..4 | TC-1231..TC-1245, TC-1816..TC-1818 | ✅ Complete |
...
| TC-295 | For every `2.0.0` document whose `nullable` members are all already booleans, `normalized` is byte-identical to the corpus canonical form of the input and adds no member; for any other `2.0.0` document, `normalized` differs only in materializing each such `nullable` as `=== true` | Property | P0 | FR-036-AC-6 | ✅ passed — conformance corpus (PR pending) |
...
| TC-815 | `make conformance` runs the `typescript-backend` command over all 114 cases with no adapter, unknown-case, duplicate-answer, case-digest or missing-answer problem, and the harness reports 114 matched cases and zero unsuppressed divergences for the slot | Integration | P0 | FR-070-AC-1, FR-070-AC-11 | 🚧 measured by `make conformance`, which `make test` does not run; no test binds this row |
...
| TC-1285 | Every document under fixtures/ is named in its PROVENANCE.json with repository, revision, and path or as authored; config-version-* rows name the quire-rs revision and the added relationships: block; the module row names 7b7b0bc | Unit | P0 | FR-098-AC-1, FR-098-CON-2 | ✅ passed |
...
| TC-1322 | quire-rs is a git dep at exact rev 08d39ea with no branch, ix-trace-rs dev at v0.1.1, semantic-ir by path, serde pins exact, sha2 and clap exact, no jsonschema, no outside path/file/link dep; module PROVENANCE names 7b7b0bc | Static | P1 | NFR-033-AC-3 | ✅ passed |
...
| TC-1348 | The provenance record's entry for the vendored spec-objects-business module carries the manifest sha256 that fixtures/modules/spec-objects-business/PROVENANCE.json records for revision 7b7b0bc | Unit | P1 | FR-095-AC-15 | ✅ passed |
...
| TC-1352 | A Rust test in crates/extraction-frontend/tests/ asserts every row of crates/extraction-frontend/fixtures/identity-cases/identity-cases.json (identity, alias, diagnosticCode, and refusal rows, including Config Version, Config_Version, created_at, minLength, package core.data, and the part _ refusing as UNSLUGGABLE_NAME, with every owner part id-shaped because only a name part is slugged) against identity.rs, and a planted row the crate fails turns the test red naming identity.rs | Unit | P0 | FR-095-AC-16 | ✅ passed |
...
| TC-1778 | The abstract `Party` renders as a trait of accessors that `Order` implements, with no struct; a `reference` targeting `Party` generates; a field naming `Party` and a redefinition changing the accessor's Rust type are refused | Unit | P0 | FR-054-AC-19 | ✅ passed |
...
| TC-1781 | The TypeScript backend refuses a type named for another's `State` or `Equals`, a field holding an abstract type, and an inherited field collision, writing no file, generates a `reference` to an abstract type, and keys an inherited field's subsets by its declaring type | Unit | P0 | FR-064-AC-26 | ✅ passed |
...
| TC-1785 | A transition emitting or a step consuming a name that is no event artifact id, one event named twice in a cell, a `Presence` column, a `Modifies:` line, an aggregate or population `Members` table (the population refusal is retired by filament-core-data#171), a transition to an undeclared state (model unavailable), an engine transition record with an unknown trigger or a dangling guard, a transition emitting an event that lowers to nothing, and `## Relationships` rows under a module declaring the `relationships` mapping each refuse the artifact with one blocking `ARTIFACT_NOT_LOWERED` naming the declaration; under a module not declaring the mapping the rows are the engine's blocking `feature-not-extractable` | Unit | P0 | FR-143-AC-6 | ✅ passed |
...
| TC-1592 | The corpus run exits clean over all 114 cases | Integration | P0 | FR-090-CON-1 | ✅ passed |
...
| ERR-260 | `availability.fields` is `unavailable` or `missing`, an enumeration's `values` locator is unsatisfied, or the engine dropped a row on `semantic.ambiguous-type` | `ARTIFACT_NOT_LOWERED` naming the engine reason; `warning` for `legacy-form`, `error` and blocking otherwise | TC-1209, TC-1213, TC-1228, TC-1333 |
...
| ERR-267 | A part slugs to the empty string (`_`, `--`), or two distinct names on one owner slug alike (`created_at` beside `created__at`; enumeration rows `a_b` and `a__b`; contract case (b)) | `UNSLUGGABLE_NAME` at the declaration, or at the later of the two; blocking; no part is dropped | TC-1252, TC-1334, TC-1352, TC-1353 |
