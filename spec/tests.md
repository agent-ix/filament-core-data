...
**Matrix coverage status: ✅ Complete. Execution status: ✅ 247 passed; TC-199 recorded by the owner decision on issue #4; TC-203..247 pass on PR #38 (TC-232 needs the poetry env, TC-242 the installed `spec-artifacts-iso` manifest); TC-248..279 pass on PR #39 (TC-274 by inspection; TC-279 needs the poetry env); TC-254 was re-asserted against FR-031-CON-1 when #40 published `@agent-ix/semantic-core`, and the suite is green on `main` again.**
