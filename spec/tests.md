...
| TC-1785 | A transition emitting or a step consuming a name that is no event artifact id, an `abstract` flag on a value object, a transition to an undeclared state (model unavailable), and `## Relationships` rows under a module declaring the `relationships` mapping each refuse the artifact with one blocking `ARTIFACT_NOT_LOWERED` naming the declaration; under module 0.4.0 the rows are the engine's blocking `feature-not-extractable` | Unit | P0 | FR-143-AC-6 | ✅ passed |
