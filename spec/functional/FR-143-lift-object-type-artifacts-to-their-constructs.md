...
- A systems reference member (a `part`'s or `port`'s `owner`, a `part`'s `declaredType`, a `port`'s `interfaceType`, a `connection` end, or an `allocation`'s `sourceElement`/`targetElement`) SHALL name a bundle-local artifact id, admitted by the same role check as every other artifact-id reference, and SHALL lower to that artifact's type identity; the frontend SHALL refuse a member naming an artifact of another package, an artifact the resolved role does not admit, or a form other than a plain artifact-id reference, the same way as a member naming no artifact of the bundle at all.
