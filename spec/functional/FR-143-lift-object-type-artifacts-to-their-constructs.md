...
- A systems reference member (a `part`'s or `port`'s `owner`, a `part`'s `declaredType`, a `port`'s `interfaceType`, a `connection` end, or an `allocation`'s `sourceElement`/`targetElement`) SHALL resolve to the bundle-local artifact id the engine's own reference resolution names, even though the engine hands that id back as its own `ix://<org>/<repo>/<id>` identity rather than the bare id a table cell carries; the frontend SHALL admit it by the same role check as every other artifact-id reference, and refuse a member the resolved artifact carries no admitted role for.
...
| FR-143-AC-11 | The `spec-objects-architecture` systems kinds `part`, `port`, `connection` and `allocation` lower `owner`, `declaredType`, `direction`, `interfaceType`, `multiplicity`, `sourceEnd`, `targetEnd`, `flowDirection`, `sourceElement`, `targetElement` and `featureOrder` from the engine's FR-075 extraction, with every reference member resolved to the referenced artifact's bundle-local id and admitted by role, regardless of the format the engine hands that reference back in. | Test (TC-1800) |
