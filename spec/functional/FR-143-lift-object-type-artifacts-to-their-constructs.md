...
- The frontend SHALL refuse, with blocking `ARTIFACT_NOT_LOWERED` (reason `other`) at the artifact head naming the rule, an artifact that:
  ...
  - has a transition or step whose `emits` or `consumes` names no event artifact id of the bundle, or names an event that lowers to nothing.
...
| FR-143-AC-6 | In the `business` fixture, a transition emitting `EN-001`, a step consuming `EV-999`, an `abstract: true` value object, a transition to an undeclared state, and `## Relationships` rows under a module declaring the `relationships` mapping each yield one blocking `ARTIFACT_NOT_LOWERED` naming the declaration and emit no type of that artifact; under module `0.4.0` the same rows are the engine's blocking `semantic.feature-not-extractable`. | Test (TC-1785) |
