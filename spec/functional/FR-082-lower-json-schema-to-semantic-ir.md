...
- The kernel package identity `agent-ix/semantic-core` at version `0.1.0`, the schema base `https://schemas.agent-ix.org/semantic-core/0.1.0/`, and `kernelDigest()` — all supplied by `scripts/build-semantic-kernel.mjs` from `packages/semantic-kernel/bundle.json`
...
- `source.identity` SHALL be the source identity of `packages/semantic-core/main.tsp`, `source.version` SHALL be `0.1.0`, and `source.digest` SHALL be `kernelDigest()`, so the document names the authored source and digests the exact bytes it was actually lowered from.
...
| FR-082-AC-12 | The emitted document declares `source.dialect` of `typespec`, `source.digest` equal to `kernelDigest()`, and `source.version` of `0.1.0`; the provenance names `packages/semantic-core/main.tsp` as the authored source, the pinned `@typespec/json-schema` `1.15.0` emitter as the projector, and the issue number of the dialect question filed with the contract owner. | Test |
