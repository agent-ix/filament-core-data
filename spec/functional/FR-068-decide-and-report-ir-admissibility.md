...
| FR-068-AC-10 | An IR carrying an operation, a clause, an unimplemented `format`, a `migration` default, and a `min` on a `duration` subject yields five declared losses naming those constructs and their owning type identities, all under the `agent-ix.typescript-backend.` prefix. | Test |
