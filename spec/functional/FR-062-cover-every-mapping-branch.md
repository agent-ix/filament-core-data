...
- The register SHALL carry one row per branch of the mapping: each `kind`, each
  ...
  diagnostic code.
- The register SHALL carry one `name-derivation` row per support scalar —
  `date`, `datetime`, `duration`, and `uuid` — contributed by `names.mjs`, each
  naming the case that exercises the mapping of a kernel-scalar definition onto
  its support type under FR-055 "Scopes and collisions"
  (agent-ix/filament-core-data#90), and each SHALL be bound to a case.
- `node src/compiler/backends/rust-serde/cli.mjs register --check` SHALL pass
  after the register is regenerated with those rows.
...
| FR-062-AC-15 | The regenerated register carries exactly four `name-derivation` rows for the support scalars `date`, `datetime`, `duration`, and `uuid`, each contributed by `names.mjs` and each naming an existing case; `cli.mjs register --check` passes after regeneration and fails naming the row when any one of the four cases is removed. | Analysis (TC-1359) |
