...
- The frontend SHALL mint identities by the rules of FR-034, rooted at the package identity rather than at `<org>/<repo>`: `ix://<package identity>/<Name>` for a type definition and `.../<Name><Field>` for the alias a constrained property mints (neither under a slot of its own), `.../<Name>/<field>` for a field and `.../<Name>/<operation>` for an operation (each nesting under its owner's identity rather than a `field/` or `operation/` slot), `.../<Name>/<operation>/<param>` for an operation parameter (nesting one level deeper than its operation, never under a `param/` slot), `.../variant/<Name>-<member>`, `.../relationship/<Name>-<verb>-<TargetName>`, `.../clause/<Name>-<clauseId>`, `.../constraint/<Name>-<field>-<keyword>` for a field constraint, `.../constraint/<Name>-<keyword>` for a type constraint, and `.../<KernelScalar>` for a kernel scalar definition.
- This rule is stated once, in `docs/semantic-data-system/contracts-v1.md` §Identity minting, and the spec-bundle extraction frontend (FR-095) implements the same rule unchanged, so the two frontends mint one identity for one declaration (issue #87).
- The `variant` slot is this requirement's addition to FR-034's list, because `semantic-ir.schema.json` requires an identity on every enum member and union variant and FR-034's lowering leaves it implicit.
...
- The frontend SHALL lower the TypeSpec core decorators `@minValue`, `@maxValue`, `@minValueExclusive`, `@maxValueExclusive`, `@pattern`, `@minLength`, `@maxLength`, `@minItems`, and `@format` to the closed IR keywords `min`, `max`, `exclusiveMin`, `exclusiveMax`, `pattern`, `minLength`, `maxLength`, `nonEmpty`, and `format`.
- The frontend SHALL carry collection uniqueness in `multiplicity.unique` and SHALL NOT also emit a `unique` constraint. Issue #34's integrity review recorded that the two are one fact with two homes and no rule choosing between them (SR-029 FND-059); an inline collection field has no sequence *definition* for a constraint to apply to, since its `typeRef` is the item type. The `unique` keyword stays in the closed vocabulary for documents other frontends produce, and the reader still checks it.
...
- Where a constraint decorator is applied to a model property, the frontend SHALL mint an `alias` type definition `ix://<package identity>/<Name><Field>` targeting the property's declared type, set the field's `typeRef` to it, and attach the constraint to the alias with `appliesTo` set to the alias identity, exactly as FR-034 requires of the semantic-core lowering.
- The frontend SHALL derive each constraint's `diagnosticCode` as `agent-ix.<slug(package name) lower-cased>.<upper snake of the slugged owner parts>_<upper snake of the keyword>`, where every `-` becomes `_` and every letter is upper-cased, so the value always matches the `diagnostic.code` pattern of `common.schema.json`.
...
- The frontend SHALL read that applicability table from the same module the FR-050 reader reads it from, so the two cannot disagree about which keyword applies to what. Measured during implementation: TypeSpec's own decorators are target-checked by the TypeSpec compiler, so with the nine mapped core decorators this frontend-side check is unreachable — every inapplicable application is refused earlier as a TypeSpec `decorator-wrong-target` error. The check is kept because the shared table is the thing under test and because a later decorator addition could reach it; the rule itself is exercised through the reader, which sees documents from every frontend.
...
| FR-053-AC-11 | `NODES_ON_NON_RECORD`, `DANGLING_CLAUSE_REF`, and `DUPLICATE_CLAUSE_ID` each fire at the decorator's locus and emit no node; `@collection(_, true)` emits `multiplicity.unique` and no `unique` constraint; an inapplicable core decorator is refused by TypeSpec itself as `TYPESPEC_COMPILE_ERROR`, and the frontend reads its applicability table from the same module as the FR-050 reader. | Test |
...
- **Downstream**: [FR-046](./FR-046-lower-typespec-to-contract-ir.md), [FR-050](./FR-050-validate-and-normalize-the-emitted-ir.md), [FR-095](./FR-095-mint-package-identity-and-provenance.md)
- **Open questions**: filament-core-data#94 — the TypeSpec frontend's two known deviations from `contracts-v1.md` §Identity minting (`identity.mjs` drops an empty part instead of raising `UNSLUGGABLE_NAME`; `lower.mjs` leaves contract case (c) to the reader's `agent-ix.semantic-ir.DUPLICATE_IDENTITY` instead of refusing it before emission)
