...
| FR-053-AC-11 | `CONSTRAINT_NOT_APPLICABLE`, `NODES_ON_NON_RECORD`, `DANGLING_CLAUSE_REF`, and `DUPLICATE_CLAUSE_ID` each fire at the decorator's locus and emit no node, and `@collection(_, true)` emits `multiplicity.unique` and no `unique` constraint. | Test |
