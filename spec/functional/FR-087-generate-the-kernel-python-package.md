...
- `PROVENANCE.json` SHALL preserve the upstream MIT attribution of `datamodel-code-generator` verbatim, naming its version `0.76.0` and its licence `MIT`, and SHALL declare the generated source `AGPL-3.0-only`. The attribution is not replaced by the AGPL header.
