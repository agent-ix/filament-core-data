...
- The oracle SHALL compute `normalized` as the FR-027 normalized serialization in the corpus canonical form `agent-ix-conformance-jcs-v1`: a `1.1.0` document materializes `multiplicity`, `presence`, and `nullable` on every field and operation parameter, and a `1.0.0` document gains no member.
...
| FR-036-AC-6 | For every `1.0.0` document in the corpus, `normalized` is byte-identical to the corpus canonical form of the input, and no member is added. | Test |
