...
- That declaration SHALL copy the row's `customSourceLicense` of `AGPL-3.0-or-later` verbatim; since `agent-ix/filament-core-data#140` relicensed the programme to `AGPL-3.0-or-later`, that value agrees with the programme's mandate and the disagreement `agent-ix/filament-core-data#57` recorded no longer exists. Silently fixing a published fixture on the way through a backend is how a contract defect stops being visible.
...
- **Open contract questions this requirement records rather than decides**: the `runtime-schema-validator` runtime dependency the committed target row declares, which is not corrected here. `agent-ix/filament-core-data#57`, the committed target contract's `AGPL-3.0-or-later` against the programme's former `AGPL-3.0-only`, was resolved by the relicensing of `agent-ix/filament-core-data#140`. `agent-ix/filament-core-data#59` records that the closed issue #9 can no longer own the contract-gap register and asks for a live owner.
