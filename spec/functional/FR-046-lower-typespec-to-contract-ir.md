...
- Where a field, indexer, variant, or operation parameter is typed by a TypeSpec built-in scalar directly, the frontend SHALL emit one package-local kernel scalar definition `ix://<package identity>/<KernelScalar>` (no slot segment of its own) carrying the extension `ix://agent-ix/semantic-core/ext/kernel-scalar` (version `1.0.0`, `required: false`, payload `{ name }`), matching FR-034, and SHALL reference it.
...
- The frontend SHALL set `multiplicity.ordered` and `multiplicity.unique` on every field: from `@collection(ordered, unique)`'s arguments where that decorator is applied, `false` otherwise, regardless of whether the property is single-valued or a collection.
...
| FR-046-AC-7 | `@collection(ordered: true, unique: true)` on a single-valued property emits `multiplicity.ordered: true` and `multiplicity.unique: true` alongside its `upper: 1` bound; a property with no `@collection` emits both as `false`; `@multiplicity(2, 1)` raises `INVALID_MULTIPLICITY`, and `@multiplicity(1)` on an optional property raises `MULTIPLICITY_CONTRADICTS_OPTIONALITY`, each at the decorator's line and column. | Test |
...
| FR-046-AC-10 | A field typed by a built-in scalar directly mints no package-local definition; its `typeRef` names the kernel scalar's native reference over the closed FR-032 set (gap 1 of FCD #199/#200), and the kernel-name table is complete against FR-032 with the rows this frontend cannot reach recorded as such. | Test |
