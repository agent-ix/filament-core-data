...
- `renderIdentity` SHALL emit `TYPE_IDENTITY_FIELDS`, mapping every `entity`, `nested_entity`, `aggregate_root` and `process` to the names of its identity fields in the order `identityFields` declares them; a type of another kind has no entry, and the map is emitted, empty, when the document declares no entity, so a consumer reads one surface.
...
- `abstract`, `subsets`, clauses, transition guards and operation frames are carried as data, not enforced: TypeScript has no abstract interface and no subset relation, and a clause's Quire meaning is over instances. `loss.mjs` names each in `RENDERED_NOT_LOST`.
