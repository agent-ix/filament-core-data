...
The producer SHALL emit a v1.1 projection of a 1.2 document, declaring on it the
v1.1 wire schema identity and the v1.1 producer interface version and emitting it
as its own document set, only when every authored presence in that document
equals the value a v1.1 reader derives and no other 1.2-only contract member
would be lost, and SHALL otherwise refuse the projection with an
identity-preserving loss record naming every member the projection would lose.
...
- The requested target interface version of the projection, and the v1.1 wire
  schema identity that target version declares, as
  [FR-126](./FR-126-declare-the-producer-interface-version.md) defines both
  members.
...
- One v1.1 projection document carrying its own identity, its own namespaced
  revision, its own digest selection, its declared v1.1 wire schema identity, and
  its declared v1.1 producer interface version, when every 1.2-only distinction
  of the source survives the projection.
- One document set containing that projection alone, distinct from the document
  set the projected 1.2 document belongs to.
...
- The producer SHALL emit a v1.1 projection of a 1.2 document only when every
  ...
  that field's multiplicity, as FR-106 defines that derived value.
...
- The producer SHALL declare the v1.1 wire schema identity on every v1.1
  projection document.
- The producer SHALL declare the v1.1 producer interface version on every v1.1
  projection document.
- The producer SHALL NOT declare the 1.2 producer interface version on a v1.1
  projection document.
- The producer SHALL emit a v1.1 projection as its own document set whose only
  member is that projection.
- The producer SHALL NOT emit a v1.1 projection as a member of the document set
  the projected 1.2 document belongs to.
- The producer SHALL treat `v1.1` as a wire schema version and a producer
  interface version only.
- The producer SHALL NOT emit `v1.1` as a model revision value.
- The producer SHALL NOT emit `v1.1` as a digest domain version.
...
| --- | --- | --- | --- |
| FR-125-CON-1 | The producer SHALL take the authored-presence semantics of a projection, including the value a v1.1 reader derives from a field's multiplicity, from the FR-106 field contract, restating no part of that contract here. | Traceability | Inspection |
...
| FR-125-CON-3 | The producer SHALL keep a projection document's identity, revision, digest selection, declared wire schema identity, and declared interface version separate members from those of the document it projects, so equal spelling never merges the two documents. | Traceability | Test |
...
| FR-125-CON-5 | The producer SHALL take the refusal of a requested target interface version it does not implement from FR-126, which owns that refusal, restating it as no obligation of this requirement. | Traceability | Inspection |
| FR-125-CON-6 | The producer SHALL keep the declared wire schema identity, the declared producer interface version, and the model revision value of a projection document three separate members, so `v1.1` never appears as a model revision value. | Integrity | Test |
...
| FR-125-AC-2 | A 1.2 document carrying one field whose authored presence differs from the value a v1.1 reader derives refuses its projection, and the blocking loss record names that field identity. | Test |
...
| FR-125-AC-9 | An admitted v1.1 projection declares the v1.1 wire schema identity and the v1.1 producer interface version, declares neither the 1.2 interface version nor `v1.1` as a model revision value, and is emitted as a document set whose only member is that projection. | Test |
...
- [FR-106](./FR-106-author-field-presence-independently.md) defines the authored
  ...
  value a projection is compared against, and owns the presence contract itself;
  this requirement states the interface obligation for emitting the projection
  and its loss record, and restates no part of that contract.
...
- [FR-126](./FR-126-declare-the-producer-interface-version.md) defines the
  producer interface version member and the wire schema identity member a
  projection declares, keeps those two distinct from the model revision and the
  digest domain version, applies its one-interface-version-per-document-set rule
  to each document set separately, and owns the refusal of a requested target
  interface version the producer does not implement; this requirement cites that
  refusal rather than re-owning it.
- Vocabulary, stated once for this requirement: `v1.1` and `1.2` here are wire
  schema versions and producer interface versions only, never model revision
  values; a projection's `selection` is its digest selection or its revision
  selection, each named where it is used.
...
- **Assumed external contract, not owned here**: the consumer contract at
  ...
  `ProducerObject` record carries the identity, revision, and digest selection of
  a projection document as it does of any other producer object, and whose own
  wire schema identity the v1.1 projection declares; this increment maps onto
  that contract rather than owning it, and the `interface` index of that record
  is assigned by the consumer rather than authored here.
