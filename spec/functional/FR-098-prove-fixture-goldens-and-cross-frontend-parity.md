...
module is vendored from repository revision `f7fdfda` (`0.4.0`, untagged
`main`), the first revision whose object types declare the FR-075 model tables
the engine gates on. That revision declares
`allowed_links` and one `edge_types` entry, `specializes`; the FR-040 registry
FR-094 reads lives in spec-artifacts-iso, whose manifest cannot be vendored whole for one registry,
...
- The spec-objects-business repository at revision `f7fdfda`: `manifest.yaml` and `schemas/`
...
|---|---|---|
| FR-098-AC-1 | Every document under `fixtures/` is named in its `PROVENANCE.json` with repository, revision, and path or as `authored`; the `config-version-*` rows name the quire-rs revision and the added `relationships:` block; the `modules/spec-objects-business` row names revision `f7fdfda`. | Test (TC-1285) |
