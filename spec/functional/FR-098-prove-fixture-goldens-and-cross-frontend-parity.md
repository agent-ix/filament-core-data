...
| FR-098-AC-3 | The `business` fixture's golden carries at least one `record`, `enum`, and `scalar` definition, one `enum` with at least two `variants`, one operation with parameters and a `returns`, one `ocl` clause, and one relationship of each of the categories `structural` and `dependency`. | Test (TC-1287) |
