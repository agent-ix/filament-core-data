...
structural projection cannot hold on `records-and-scalars`: the two frontends
mint identities by different closed rules (FR-053 uses the declaration name,
FR-095 the lowercase slug; the TypeSpec author names a scalar where FR-093
mints the alias `type/<DisplayName>.<fieldName>`; `diagnosticCode` prefixes
differ), so no projection over identities agrees without rewriting one
frontend's identity rule. That is filed as filament-core-data#87 and
`records-and-scalars` is recorded single-dialect with that reason until #87
...
`BACKEND_NOT_IMPLEMENTED`; the Rust backend's entry is
`src/compiler/backends/rust-serde/cli.mjs`, which the Rust half of the backend
acceptance runs through. The FR-045 node seam
...
- `test/fixtures/compiler/shared/cases.json`: the `records-and-scalars` case with both source trees present and `"spec-bundle": null` with a `reason` naming filament-core-data#87, and for each existing case `"spec-bundle": null` with a `reason`
...
- The frontend SHALL compare the two dialects' projections byte for byte after `agent_ix_semantic_ir::normalize::normalized`, once filament-core-data#87 decides a shared identity rule; until then the comparison is blocked and the parity test asserts zero two-dialect cases.
- The frontend SHALL record `records-and-scalars` in `cases.json` with both source trees present and `"spec-bundle": null` with a `reason` naming filament-core-data#87 and the identity rules that differ (declaration name against lowercase slug, author-named scalar against the minted alias, `diagnosticCode` prefixes).
...
- The frontend SHALL emit a `config-version-table` document that `node src/compiler/backends/rust-serde/cli.mjs` (the Rust backend's entry; `node src/compiler/cli.mjs generate --target rust` is `BACKEND_NOT_IMPLEMENTED`) accepts with zero diagnostics.
...
| FR-098-AC-4 | Every code FR-096 declares is emitted by its `negatives/<CODE>/` bundle or by the test `constructed.json` names, at the golden's recorded line and column or with no locus, as exactly its code as the first blocking diagnostic; companion diagnostics that the same defect necessarily produces (an `ENGINE_DIAGNOSTIC` row, an `ARTIFACT_NOT_LOWERED`) are pinned by the test; the non-blocking negatives are exactly two, `ENGINE_DIAGNOSTIC` and `ARTIFACT_NOT_LOWERED` (legacy form), `KERNEL_NAME_SHADOWED`'s fixture being refused with `DUPLICATE_TYPE_NAME` at lift level per FR-092-AC-8. | Test (TC-1288) |
...
| FR-098-AC-8 | `node src/compiler/backends/rust-serde/cli.mjs` over the lifted `config-version-table` document exits zero with zero diagnostics; `node src/compiler/cli.mjs generate --target typescript` over the same document exits zero with zero diagnostics once filament-core-data#88 is fixed (until then it refuses the document with `DUPLICATE_IDENTITY` and that half of TC-1292 is blocked). | Test (TC-1292, TypeScript half blocked on filament-core-data#88) |
