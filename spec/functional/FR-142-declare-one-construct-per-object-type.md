...
| `references` | reference member → the roles a named type carries to be admitted; the reference members are `owner`, `members`, `persists`, `interfaceType`, `declaredType`, `sourceElement`, `targetElement`, `sourceEnd` and `targetEnd` (each end's `type`), and `transitions` (each transition's `emits`) and `steps` (each step's `consumes` and `emits`); `sourceElement` also admits an identity naming an operation, checked by the roles of the type that declares that operation |
...
| `identityRequirements`, `shapeRequirements` | The member presence a declaration of a stated identity or shape declares: `identified` requires `identityFields`, `enumeration` requires `variants` |
| `flags` | An optional boolean declaration member with its default: `immutable`, default `false` |

The Rust reader `crates/semantic-ir` is the one authority for the construct
rules: it implements the rules of every construct kind once, generically over
declarations, and it alone decides a rule at an artifact or a document. The
Node compiler and the Python backend read the same vocabulary file and the
published schema, and check a document against the schema layer only — the
closed terms, member presence, and a list member a `nonEmpty` rule requires —
so no rule is implemented twice. No reader, frontend or backend matches on a
construct kind's name; backends dispatch on a construct's `shape` and
`identity`.

A declaration also carries the optional flags the vocabulary states, each a
boolean with its default: `immutable` states that an instance of the construct
never changes once created, and a declaration that does not state it declares a
mutable construct.
...
- The schema SHALL refuse a construct missing a member its declaration requires, carrying a member its declaration forbids, or carrying an empty list where a rule the vocabulary marks `nonEmpty` applies, with `SCHEMA_VIOLATION`; a core-kind type SHALL NOT carry a member whose default is `forbidden`, except an alias's `target`.
- The Rust reader SHALL raise `UNRESOLVED_CONSTRUCT_REF` for a reference member entry naming no declared type (for `sourceElement`, no declared type and no operation of a declared type), and for a member naming no declared state, operation or field.
- The Rust reader SHALL raise `CONSTRUCT_TARGET_KIND` for a reference member entry naming a type that carries none of the roles its declaration admits, for a supertype of another kind, and, under `members_not_namespace`, for a member whose construct's shape is `namespace`.
- The Rust reader SHALL raise `INVALID_OCCURRENCE_FIELD` for an occurrence field not resolving to scalar `datetime`.
...
- The Rust reader SHALL raise `UNRESOLVED_CONSTRUCT_REF` for a `featureOrder` entry naming no single field or operation its type declares itself, and `INCOMPLETE_FEATURE_ORDER` for each such field or operation a `featureOrder` omits; a backend that renders no feature order SHALL ignore it.
- The Rust reader SHALL raise `MULTIPLE_DOMAIN_MEMBERSHIP` for a type named by the members of two types whose declarations select `exclusive_membership`.
- The Rust, Node and Python readings of the core vocabulary SHALL come from `construct-vocabulary.json`, naming no term, member, rule or flag of their own, and SHALL equal the schema's `constructDeclaration`: the member defaults, the reference items, each rule's required member presence and each flag's default included.
- Reading a declaration SHALL refuse an identity or shape whose required member presence the declaration does not declare, and a flag that is not a boolean, at the pointer of the identity, shape or flag; the Rust and Node readings SHALL refuse the same declaration at the same pointer.
...
| FR-142-AC-3 | An owner, an aggregate member and a persisted type carrying none of the admitted roles, a namespace member that is a namespace, and a reference member naming no type each raise their reader code at the member pointer; a `sourceElement` naming an operation of a declared type is admitted, the same identity named by `targetElement` is refused, and a `sourceElement` naming neither a declared type nor a declared operation is refused. | Test (TC-1747) |
...
| FR-142-AC-9 | A type definition's or a population's kind naming no `constructs` entry, an entry no type or population uses, a kind declared twice, a `2.0.0` document without `constructs`, a wildcard role and a rule whose member presence is undeclared are each refused with `SCHEMA_VIOLATION`; widening a declaration's admitted roles admits a reference its former roles refused, and removing the role from the named type refuses it; a `sourceElement` resolved through FR-152's operation fallback is checked by the same role admission, against the role of the operation's owning type. | Test (TC-1789) |
...
| FR-142-AC-11 | The Rust, Node and Python readings of the core vocabulary each equal `construct-vocabulary.json` and the schema's `constructDeclaration`, member defaults, reference items, rule requirements and flag defaults included. | Test (TC-1786) |
...
| FR-142-AC-14 | An `identified` declaration declaring no required `identityFields`, an `enumeration` declaring no required `variants`, and a flag that is not a boolean are each refused at the pointer of the identity, shape or flag by the Rust and the Node reading alike; a declaration stating no flag reads it at its default. | Test (TC-1786, TC-1788) |
