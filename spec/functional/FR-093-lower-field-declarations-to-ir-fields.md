...
`typeDefinition` (`kind: record`, or `kind: enum` for an enumeration), plus
...
- If a document's `displayName` equals the name of a kernel scalar the bundle uses (FR-092), then the frontend SHALL raise blocking `DUPLICATE_TYPE_NAME` at that document naming the kernel scalar, because the minted `type/<Name>` collides with the FR-092 scalar definition.
...
- If a `FieldDecl` carries one or more constraints, then the frontend SHALL mint one `typeDefinition` of `kind: alias` at `ix://<org>/<name>/type/<DisplayName>.<fieldName>` (FR-095) with `displayName` `<DisplayName>.<fieldName>`, `target` the FR-092 resolution of the field's type (a kernel scalar, record, or enum definition; never another alias), `roles: []`, `unknownPolicy: reject`, `extensions: []`, `origin.source` the row's locus, and the row's constraints as its `constraints[]`; the field's `typeRef` SHALL then be the alias identity. This is FR-034's alias-per-constrained-field form (`VersionNumber` in `fixtures/semantic/v1/positive/config-version-v1-1.json`).
...
- The frontend SHALL lower each `Constraint` to one IR `constraint` under FR-029 with `appliesTo` the alias identity, `identity` `constraint/<record-slug>-<field-slug>-<keyword>` (FR-095), `diagnosticCode` `agent-ix.<name>.<SCREAMING_FIELD>_<KEYWORD>` where `<KEYWORD>` is the constraint keyword in SCREAMING_SNAKE (`maxLength` → `MAX_LENGTH`), and the row's origin; `pattern` carries `{regex, dialect}`, `enumValues` carries `{values}`, `format` carries `{name}`, and the bound keywords carry `{value}`.
...
- The frontend SHALL emit one `DECLARED_LOSS` naming `required-collection-presence` per collection field declared `0..*` or `*`; no module marker distinguishes a required-but-possibly-empty collection, so every such field is the loss.
...
is the resolved type. The alias identity `type/<DisplayName>.<fieldName>` is
admitted by `semanticIdentity` and cannot collide: two fields of one record
cannot share a name (the engine's `DUPLICATE_FIELD`), and a record
`displayName` is an `Identifier`, which never contains a dot.
...
| FR-093-AC-4 | `versionNumber | Integer | 1 | min: 1` emits one `kind: alias` definition `type/ConfigVersion.versionNumber` with `target` `type/Integer`, `roles: []`, `unknownPolicy: reject`, origin at the row, and one `min` constraint with `operands.value: 1`, `appliesTo` the alias identity, and `diagnosticCode` `agent-ix.<name>.VERSION_NUMBER_MIN`; the field's `typeRef` is the alias identity, the record's `constraints` is `[]`, and no unconstrained field mints an alias. | Test (TC-1223) |
| FR-093-AC-5 | `createdBy | String | 1 | maxLength: 64` emits `maxLength` with `{value: 64}` on the alias `type/ConfigVersion.createdBy` targeting `type/String`; a `pattern /^[a-z]+$/` cell emits `{regex, dialect: "ecma-262"}`; an `enumValues a\|b` cell emits `{values: ["a","b"]}`; a row carrying two keywords yields one alias carrying both constraints. | Test (TC-1224) |
