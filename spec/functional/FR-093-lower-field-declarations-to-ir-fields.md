...
- The frontend SHALL lower a `JsonObject` target to one package-local `kind: scalar` definition with `scalar: any` at `ix://<org>/<name>/JsonObject` (FR-139), with no declared loss.
