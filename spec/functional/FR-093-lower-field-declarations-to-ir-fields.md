...
- A kernel scalar mints no package-local node to collide with (gap 1 of FCD #199/#200, FR-092), so a document whose `displayName` equals a kernel scalar name raises no `DUPLICATE_TYPE_NAME` for that reason: the frontend SHALL lower it to its own `type/<Name>` definition exactly as it would any other `displayName`, and the artifact is unreferenceable by that name in a `Type` cell only because the engine resolves the cell to the kernel scalar first (FR-092's `KERNEL_NAME_SHADOWED`).
...
- If `FieldDecl.identity` is `true`, then the frontend SHALL carry it as the extension `ix://agent-ix/semantic-core/ext/identity-field` (version `1.0.0`, `required: false`, payload `{}`); the IR declares no `identity` member on a field.
...
- The frontend SHALL lower a `JsonObject` target to the native reference `ix://quire/native/JsonObject` with no declared loss and no package-local node minted (FR-139, gap 1 of FCD #199/#200).
...
| FR-093-AC-7 | A `JsonObject` cell's field carries `typeRef` `ix://quire/native/JsonObject` directly, no `JsonObject` type node or field carrying a `scalar` member is emitted, and no `DECLARED_LOSS` is raised; `losses.json` carries no `unconstrained-value` row. | Test (TC-1226) |
