...
- The publication-gate record in `docs/semantic-data-system/semantic-kernel-packages.md`, naming `agent-ix/quoin#290`, alongside the inherited `agent-ix/filament-core-data#21` defect this requirement does not fix
...
- `scripts/build-semantic-kernel.mjs` SHALL generate the kernel crate by `src/compiler/backends/rust-serde/cli.mjs generate --out <dir>` over a compiler request carrying `packages/semantic-kernel/semantic-ir.json`, the declared `BACKEND` identity `ix://agent-ix/filament-core-data/rust-backend`, and the default profile and limits `cli.mjs` already declares, so the kernel is generated by the route the corpus bases take.
- The manifest for a completed kernel generation SHALL carry `state: "success"`, one `files[]` entry per emitted file, and no blocking diagnostic. This is the measured result recorded in the issue #11 baseline: `state: "success"` and 61 files.
...
- The generated crate declares no `Cargo.lock` of its own, so the resolve SHALL be satisfied entirely from the offline supply named in Inputs; a resolve that would reach a registry SHALL fail the gate rather than fetch.
...
- The two-artifact discipline is carried here because it is what failed on `agent-ix/filament-core-data#21`, and the failure is named rather than assumed away. `make rust-check` is red on `main`: both `test/fixtures/rust-serde/goldens/**` and `test/fixtures/rust-serde/digests.json` are stale relative to `crate.mjs`. The redness was reproduced on a clean `git clone` under `mktemp -d` at `65ea7fa` and again at #21's own merge commit `89e0ea1`, so it is a property of the repository and not of a working tree. The two artifacts did what they were built to do — they both went red — and what went wrong is that they were left red.
- This requirement SHALL NOT repair `test/fixtures/rust-serde/goldens/**` or `test/fixtures/rust-serde/digests.json`. Those artifacts belong to the reopened `agent-ix/filament-core-data#21`, regenerating them here would be this ticket blessing another ticket's emitter output, and the consequence is stated plainly: `make test` cannot be green on any branch cut from `main` while #21 stands.
...
| FR-086-AC-10 | This branch leaves `test/fixtures/rust-serde/goldens/**` and `test/fixtures/rust-serde/digests.json` byte-unchanged, and `make semantic-kernel-check` neither invokes nor depends on `make rust-check` nor shares a scratch directory or `CARGO_TARGET_DIR` with any `rust-*` target; the reopened `agent-ix/filament-core-data#21` redness on `main` is recorded, with its two clean-clone reproductions at `65ea7fa` and `89e0ea1`, and is not repaired here. | Analysis |
...
| FR-086-AC-18 | `make semantic-kernel-check` reports on every run that Rust publication is blocked on `agent-ix/quoin#290`, that `publish = false` in the generated manifest is what enforces it, and that the `agent-ix/filament-core-data#21` redness is an inherited open defect this ticket does not fix; a blocked gate carrying no owning issue fails the check. | Inspection |
...
| FR-086-AC-20 | `docs/semantic-data-system/semantic-kernel-packages.md` records Rust publication as blocked on `agent-ix/quoin#290` with `publish = false` named as what enforces it, and records `agent-ix/filament-core-data#21` as an inherited open defect with its two clean-clone reproductions; a row carrying no owning issue fails the gate. | Inspection |
...
- **Inherited open defects this requirement records rather than closes**: `agent-ix/filament-core-data#21`, reopened — `make rust-check` is red on `main` because `test/fixtures/rust-serde/goldens/**` and `test/fixtures/rust-serde/digests.json` are both stale relative to `crate.mjs`, reproduced on clean clones at `65ea7fa` and `89e0ea1`, so `make test` cannot be green on any branch cut from `main`. Publication is blocked on `agent-ix/quoin#290`.
