...
relationships:
  ...
  - target: "ix://agent-ix/filament-core-data/FR-108"
    type: "depends_on"
---
# FR-109: Declare ecosystem configuration contracts
...
- Versioned model/profile/configuration selections and declared assessment inputs.

## Outputs

- An immutable static closure and, when selected, a separately bound assessment input set.
...
| --- | --- | --- | --- |
| FR-109-CON-1 | A semantic result SHALL retain the exact configuration identity it consumed. | Traceability | Test |
| FR-109-CON-2 | A configuration change SHALL produce a distinct semantic input identity. | Integrity | Test |

## Acceptance Criteria

| ID | Criteria | Verification |
| --- | --- | --- |
| FR-109-AC-1 | An unknown profile reference is refused before model or population evaluation. | Test |
| FR-109-AC-2 | Changing only a resource-limit value changes the configuration identity and remains observable in the result. | Test |
| FR-109-AC-3 | A consumer with no explicit configuration refuses rather than selecting a current-directory or environment default. | Test |
| FR-109-AC-4 | A clause package links against an exact model/profile/configuration closure with no population records, while an assessment needing an absent selected window reports its own missing-input disposition. | Test |
| FR-109-AC-5 | A producer canonical-object digest and a native raw-byte digest both validate only in their named domain; substituting either for the other refuses the binding. | Test |

## Dependencies

- [US-015](../usecase/US-015-lift-a-spec-bundle-into-a-domain-package.md) supplies the compilation/extraction outcome.
- [FR-106](./FR-106-author-field-presence-independently.md) and [FR-108](./FR-108-bind-populations-to-model-contracts.md) supply the selected contracts.
