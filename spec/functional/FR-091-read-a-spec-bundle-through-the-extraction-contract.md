...
- The frontend SHALL hand each recorded `SemanticDiagnostic` to FR-096, which wraps it as one `ENGINE_DIAGNOSTIC` whose `causes[0]` reproduces the engine `code`, `message`, and `reason`, and whose severity is the FR-096 mapping of the engine severity.
