...
`typeIdentity` naming a type the bound static bundle exports, its field-member
states keeping absence, explicit null, and a present value three different
outcomes, and its relationship-instance endpoint identities, under a document
that declares explicitly whether its world is closed.
...
- The bound admitted static bundle of
  [FR-117](./FR-117-admit-a-static-producer-bundle.md), whose exported type
  identities are the available type vocabulary, bound as
  [FR-120](./FR-120-bind-an-assessment-document-to-a-static-bundle.md)
  requires.
- The declared object universe against which a closed-world population is
  checked.

## Outputs

- One population document carrying its declared `closedWorld` value and a
  finite `members` set whose members are each separately readable typed records.
- A blocking refusal naming the offending object identity, type identity,
  field-member state, or relationship-instance endpoint identity when the
  membership cannot be emitted.
...
- The producer SHALL emit, as every member's `typeIdentity`, a type identity
  that the bound static bundle exports.
...
- If the population document declares a closed world and a member's
  `objectIdentity` lies outside that declared universe, then the producer SHALL
  refuse the population document, naming that object identity.
- If a member's `typeIdentity` names a type the bound static bundle does not
  export, then the producer SHALL refuse the population document, naming that
  type identity.
...
| FR-121-CON-4 | The producer SHALL resolve every member `typeIdentity` against the exported type vocabulary of the bound static bundle and against no other vocabulary. | Interface | Test |
| FR-121-CON-5 | The producer SHALL keep membership of an object and coverage of an observation record distinct, so one member never claims a record's coverage. | Correctness | Test |
...
| FR-121-AC-2 | A member whose `objectIdentity` lies outside the declared universe of a closed-world population refuses with a blocking refusal naming that object identity. | Test |
| FR-121-AC-3 | A member whose `typeIdentity` names a type the bound static bundle does not export refuses with a blocking refusal naming that type identity. | Test |
...
- [FR-120](./FR-120-bind-an-assessment-document-to-a-static-bundle.md)
  binds this population document to one admitted static bundle, which supplies
  the exported type vocabulary each member's `typeIdentity` is resolved against.
...
- The consumer contract this requirement maps onto is an assumed external
  ...
  the assessment `ExportKind::Population` export kind and the
  `BindingRequirement` record with its `BindingKind` members `population`,
