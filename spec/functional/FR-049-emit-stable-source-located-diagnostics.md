...
| FR-049-CON-4 | The `agent-ix.semantic-ir.*` spellings SHALL be exactly those the issue #34 readers already emit, extracted from `fixtures/semantic/v1/negative/reader-cases.json` and `test/semantic-ir-v1-1-reader.ts` (fcd#179 deleted contracts 1.0.0 and 1.1.0, which those readers also read, and edited both files to drop the cases and spellings that went with them; the extraction runs against their current bytes, not a byte-unchanged snapshot). | Consistency | Test |
