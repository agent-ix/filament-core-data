...
the engine's own loaders (plus each module root's `manifest.yaml`, read by
`write::read_manifest` in `write.rs` for FR-095's digests and for the construct declarations of FR-142, FR-091-CON-2), and nothing else. The cargo target directory is named
