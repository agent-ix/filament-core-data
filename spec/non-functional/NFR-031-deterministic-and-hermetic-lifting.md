...
| Byte difference between the table-form and fence-form lift of one declaration at one bundle-relative path | 0 | 0 | Cross-form comparison over two bundle roots |
...
roots, whose copies of `FR-006` sit at the same bundle-relative path, and
compare; cite `quire-rs/src/corpus/walk.rs` for path-sorted loading and the
...
| NFR-031-AC-3 | The `config-version-table` and `config-version-fence` bundle roots, each holding one copy of `FR-006` at `spec/functional/FR-006-config-version-entity.md`, lift to identical bytes. | Test (TC-1302) |
