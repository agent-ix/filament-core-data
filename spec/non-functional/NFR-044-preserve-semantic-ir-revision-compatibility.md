...
while they admit contract revision `1.2.0`.
...
- Excludes a document authored as `1.2.0`, which carries the `any` scalar,
  authored presence, the model members of FR-141 or the constructs of FR-142.

## Rationale

Contract `1.2.0` is additive (issue #93, issue #146). A consumer that reads
`1.1.0` documents receives the same valid document and the same fingerprint,
and a `1.2.0` node inside a `1.1.0` document is refused, never read as a
`1.1.0` node.
...
| `1.1.0` to `1.2.0` classification | Additive | Additive | Compatibility test |
...
compatibility classifier classifies the `1.1.0` to `1.2.0` uplift.

## Acceptance Criteria

| ID | Criteria | Verification |
|---|---|---|
| NFR-044-AC-1 | Every published `1.0.0` and `1.1.0` fixture keeps its Node, Python and Rust reader verdict and its canonical bytes under the `1.2.0` schema, and a `1.1.0` document carrying any `1.2.0` node is refused with `SCHEMA_VIOLATION`. | Test (TC-1756) |
| NFR-044-AC-2 | The compatibility classifier reports the `1.1.0` to `1.2.0` uplift of one document as additive. | Test (TC-1757) |
