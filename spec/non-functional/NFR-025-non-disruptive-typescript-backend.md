...
- Permitted paths: `spec/**`, `plan/**`, `reviews/**`, `test/**`, `tests/**`,
  ...
  `src/compiler/backends/typescript-v1/**`, `src/compiler/cli.mjs`,
  ...
  and `conformance/divergences.json`.
- Prohibited paths: every other path, and in particular `schema/**`,
  ...
  `conformance/mutations.json`, `conformance/diagnostic-codes.json`, and
  `conformance/schema/**`.
...
dependency blocks against the merge base. Assert the narrow build interface
...
| NFR-025-AC-3 | `package.json` `exports`, `main`, `module`, `types`, `files`, `dependencies`, `peerDependencies`, and `optionalDependencies` are byte-unchanged from the merge base. | Analysis |
| NFR-025-AC-4 | `pnpm-lock.yaml` and `poetry.lock` are byte-unchanged from the merge base. | Analysis |
...
| NFR-025-AC-6 | `src/compiler/backends/typescript.mjs`, `rust.mjs`, and `type-names.mjs` and the four issue #4 goldens are byte-unchanged, and the golden comparisons still pass. | Snapshot |
