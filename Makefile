...
# -----------------------------------------------------------------------------
# Baseline producer NFR-036 evidence (issue #95, Plan-017 Task-150)
# -----------------------------------------------------------------------------
# NFR-036 names Plan-017 the owner of the apparatus its gates need: the second
# architecture of the named set and the runner that provides it, the
# instrumented ambient-read run, and the unprivileged network namespace. These
# targets are that apparatus.
#
# The named architecture set is exactly `x86_64-unknown-linux-gnu` and
# `aarch64-unknown-linux-gnu`. No third architecture is claimed.
#
# `baseline-producer-record` runs the recording control on whatever architecture
# the host is, writing that architecture's agreement record.
# `baseline-producer-second-architecture` builds and runs the same control for
# the second architecture, naming its linker and its runner explicitly: a gate
# that ran on whatever `cargo` resolves to would measure the host twice and
# report agreement with itself. `baseline-producer-cross-architecture` then runs
# the `#[ignore]`d agreement controls, which compare the two records element by
# element and fail naming the architecture whose record is absent.
#
# Every step fails rather than skips. A missing `rust-std`, a missing cross
# linker or a missing runner is a red gate naming what was missing, which is the
# reading NFR-036's Verification requires: "a gate that cannot run fails saying
# so rather than passing".
#
# `baseline-producer-ambient-evidence` runs the `#[ignore]`d instrumented
# ambient-read control: it compiles `tests/probe/ambient_probe.rs`, re-invokes
# the test binary under `unshare -rn` with that instrument preloaded, and plants
# a clock read and an environment read inside the measured region to prove the
# instrument records rather than assumes. It nests `cargo`, `rustc` and
# `unshare`, which is why it is `#[ignore]`d and has a named producer here
# (the precedent `extraction-frontend-evidence` sets).

BASELINE_CRATE := agent-ix-baseline-producer
NFR036_SECOND_ARCHITECTURE := aarch64-unknown-linux-gnu
...
baseline-producer-record: rust-toolchain-check
	cargo test --offline --locked -p $(BASELINE_CRATE) --test cross_architecture -- --nocapture

.PHONY: baseline-producer-second-architecture
baseline-producer-second-architecture: rust-toolchain-check
	@rustup target list --installed | grep -qx $(NFR036_SECOND_ARCHITECTURE) || { echo "the Rust standard library for $(NFR036_SECOND_ARCHITECTURE) is not installed (rustup target add $(NFR036_SECOND_ARCHITECTURE)): the second architecture of the set NFR-036 names could not be reached, and this is a failure rather than a skip"; exit 1; }
	@command -v $(NFR036_SECOND_LINKER) >/dev/null || { echo "the cross linker $(NFR036_SECOND_LINKER) is not on PATH: $(NFR036_SECOND_ARCHITECTURE), the second architecture of the set NFR-036 names, could not be reached, and this is a failure rather than a skip"; exit 1; }
	@command -v $(NFR036_SECOND_RUNNER) >/dev/null || { echo "the runner $(NFR036_SECOND_RUNNER) is not on PATH: $(NFR036_SECOND_ARCHITECTURE), the second architecture of the set NFR-036 names, could be built but not run, and this is a failure rather than a skip"; exit 1; }
	CARGO_TARGET_AARCH64_UNKNOWN_LINUX_GNU_LINKER=$(NFR036_SECOND_LINKER) \
	CARGO_TARGET_AARCH64_UNKNOWN_LINUX_GNU_RUNNER=$(NFR036_SECOND_RUNNER) \
	cargo test --offline --locked --target $(NFR036_SECOND_ARCHITECTURE) -p $(BASELINE_CRATE) --test cross_architecture -- --nocapture

.PHONY: baseline-producer-cross-architecture
baseline-producer-cross-architecture: baseline-producer-record baseline-producer-second-architecture
	cargo test --offline --locked -p $(BASELINE_CRATE) --test cross_architecture -- --ignored --nocapture

.PHONY: baseline-producer-ambient-evidence
baseline-producer-ambient-evidence: rust-toolchain-check
	@command -v unshare >/dev/null || { echo "unshare is not on PATH: the unprivileged network namespace NFR-036-M-7 requires cannot be created, so the instrumented ambient-read run did not run, and this is a failure rather than a skip"; exit 1; }
	cargo test --offline --locked -p $(BASELINE_CRATE) --test ambient_audit -- --ignored --nocapture --test-threads=1 --exact $(BASELINE_AMBIENT_TEST)

.PHONY: baseline-producer-evidence
baseline-producer-evidence: baseline-producer-ambient-evidence baseline-producer-cross-architecture
