...
| FND-114 | low | Resolved after SR-036: the census uses portable `workspace://` repository locators; optional filesystem resolution is rooted at `FILAMENT_CENSUS_WORKSPACE_ROOT`. The suite is host-independent and passes 87 / 87. | test/contract-census.test.ts, audit/filament-contract-census/snapshot.json | implementation-bug-despite-evidence |
