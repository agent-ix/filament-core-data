...
| FND-114 | low | Pre-existing, host-bound failure recorded and not fixed: `contract-census.test.ts > resolves every source locus at the recorded repository` expects `/Users/peter/...` paths from `audit/filament-contract-census/snapshot.json` (commit `3832e07`, issue #17). The snapshot is untouched by this branch. Suite result is 86 / 87 with this as the only failure. | test/contract-census.test.ts, audit/filament-contract-census/snapshot.json | correct-requirement-no-evidence |
