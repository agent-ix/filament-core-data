...
TC-280..319 and TC-398..419 row is unbacked. The provenance is real: 122 of 122 `derivedFrom`
...
plan-creation entry (FND-614). Second, a band of test cases assert nothing that
can fail. TC-288, TC-408, TC-415 and TC-416 are tautologies; TC-282, TC-284,
TC-318, TC-402 and TC-404 are tautologies in their negative half; and TC-305
...
changed-path gate at all (FND-616, FND-617), and PRES-010 — the case that is
...
substring bug it names, which TC-318's vacuous assertion cannot see (FND-615).
...
the `unsupportedBy` accept branch are dead in both suites (FND-622).

Finding ids run FND-614..FND-409 as requested; the highest id in use elsewhere
in `reviews/` and `spec/reviews/` is FND-587, so the ranges do not collide.

## Verdict

**FAIL** — eight of the nine tasks are `pending` with the implementation
committed (FND-614), and four findings are `high`: a reproducing case that does
not reproduce its defect (FND-615), an absent changed-path and manifest-diff
gate whose three matrix rows cannot fail (FND-616), and an absent corpus
versioning gate leaving FR-035-AC-8 and NFR-015-AC-5 undischarged (FND-617).
...
- Reconciliation: **engine**, not a grep fallback. `quire coverage --scope
  ...
  `groups[spec/tests.md] = 62 / 341`, and **zero** of TC-280..319 and TC-398..419 appear in
...
- Tasks done: **0 / 9 by status**, 8 / 8 by tree (FND-614). Task-056 is the gate
...
- Issue #20 Test Cases backed by a named test: **62 / 62**. TC-280..339 and
  TC-419 are named in `it(...)` titles in `test/conformance-corpus.test.ts`
  (92 cases, several rows carrying two or three tests each); TC-418 is named in
  four `def test_tc340_*` functions in `tests/test_conformance_corpus.py`, which
  also names TC-283, TC-288, TC-289, TC-300, TC-314, TC-318, TC-403, TC-408,
  TC-411, TC-414 and TC-419. Matrix-only: 0. Tag-only: 0. **Rows backed by an
  assertion that cannot fail: TC-288, TC-408, TC-415, TC-416 (whole row);
  TC-282, TC-284, TC-291, TC-305, TC-318, TC-402, TC-404, TC-417, TC-419
...
- Acceptance criteria discharged: **43 / 57**. Fully discharged: FR-035-AC-1,
  ...
  neighbouring test, not by the row that owns them: FR-035-AC-9 (FND-618),
  FR-036-AC-2 (FND-626), FR-036-AC-8 (FND-628). **Undischarged: FR-035-AC-3
...
- Underspecified code — every file under `conformance/` traces to a
  ...
  **Two files own no requirement text: `tools/refresh.mjs`** (rewrites the
  ...
  exception of the `void`ed setup in TC-305, which is FND-620.
- Semantic review: **run**, as requested. Intent↔test↔code was judged for
  US-008 and for each of FR-035..039, NFR-015 and NFR-016; FND-615, FND-621,
  FND-622, FND-627 and FND-630 came from that pass rather than from the
...
1. Fix PRES-010 so it discriminates DEF-PROTO-004 — a type whose rendered name
   ...
   give TC-318 an assertion that names the defect's own code (FND-615, FND-621).
2. Implement the changed-path and manifest-diff gates Task-048 lists, and make
   TC-416, TC-417 and TC-419 diff the branch against `main` (FND-616).
3. Implement the versioning gate FR-035-AC-8 and NFR-015-AC-5 require — an
   ...
   `corpusVersion` bump fails — and replace TC-415's README substring
   assertion (FND-617).
4. Replace the tautologies with exercises of the gates that already exist:
   ...
   TC-408 (patch the registry, call `run()`), TC-305 (patch the registry to
   `available` and assert the `unavailable` problem), TC-404 and TC-402
   (FND-618..FND-620, FND-623, FND-624), TC-282 and TC-284 (FND-625).
5. Delete TC-291's in-process locale assignment in favour of the TC-412 form
   (FND-626), and add the reordered seed or record in the TC-303 row why the
   `single-violation` gate makes one unexpressible (FND-627).
6. Flip Task-048..055 to `done`, check their subtasks, fill the Task File
   ...
   (FND-614); reconcile the plan's `package.json` and permitted-path text with
   NFR-016 (FND-629); correct the pytest count in the Test Execution Summary
   (FND-632).
7. Then run `/code-review`, open the PR, and post the "mergeable" comment
   citing this SR-066 and the code review.
