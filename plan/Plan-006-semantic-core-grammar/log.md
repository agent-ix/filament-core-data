...
* **2026-09-03** — Task-047: SR-045 code review CONDITIONAL (check-mode ENOENT and duplicate import fixed; poetry dependency of TC-279 accepted). SR-046 gap analysis CONDITIONAL, no unmapped TC/AC, no unowned code; applied FND-212 (`EnumValue.doc` now lowers to the enum definition’s `doc` extension since IR variants carry no extensions — FR-034, `lowering.json`, lowerer, new enum-kind case), FND-215 (NFR-014 scope names the compatibility corpus path and prohibits `package.json`), FND-216 (matrix rows flipped to passed), FND-219 (stale describe label). Recorded: FND-211 (Quire coverage binds tags to the file container, CR-061), FND-213/214/217/218. PR #39; "mergeable" comment posted.
