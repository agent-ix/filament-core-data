...
- The target contract's `runtimeDependencies` names `runtime-schema-validator` while the generated package's third-party runtime dependency count is zero, and its licence string was `AGPL-3.0-or-later` where the programme then mandated `AGPL-3.0-only` — issue #57, resolved by the relicensing of #140. Both are the committed fixture's wording and are recorded here rather than silently corrected.
