...
- [x] **StR-001:** Keep the semantic data system governed while its generators become owned code.
- [x] **US-009:** Let the package tickets build from an owned, tested, deterministic compiler instead of a frozen spike.

### Functional Requirements

- [x] **FR-040:** A written disposition for each of the fourteen enumerated prototype components, plus an `authored` ledger for files the promotion writes.
- [x] **FR-041:** The semantic-IR emitter under `src/compiler/`, behind one six-symbol build interface, with `generator` and `baseDir` as explicit inputs and a locale-independent ordering.
- [x] **FR-042:** The TypeScript and Rust backends as pure IR-to-source functions reproducing the committed issue #4 goldens, with missing-base and cycle guards.
- [x] **FR-043:** The Python JSON Schema adapter with its closed forbidden-key set and pinned generator constants.
- [x] **FR-044:** The frozen spike replays through the promoted compiler with exactly one declared evidence delta and a seeded Rust lockfile.

### Non-Functional Requirements

- [x] **NFR-017:** Byte-identical output for identical inputs, with cwd, locale, and transitive resolution supplied explicitly.
- [x] **NFR-018:** No publication, no consumer move, no export-surface change, and a clean restore.
...
|---|---|---|---|
| Task-060 | [Task-060](./tasks/Task-060-guards-and-red-suite.md) | done | A |
| Task-061 | [Task-061](./tasks/Task-061-promoted-emitter-and-build-interface.md) | done | A |
| Task-062 | [Task-062](./tasks/Task-062-language-backends.md) | done | B |
| Task-063 | [Task-063](./tasks/Task-063-python-schema-adapter.md) | done | B |
| Task-064 | [Task-064](./tasks/Task-064-spike-rewiring-and-declared-delta.md) | done | A |
| Task-065 | [Task-065](./tasks/Task-065-promotion-inventory.md) | done | A |
| Task-066 | [Task-066](./tasks/Task-066-determinism-and-non-disruption-gates.md) | done | A |
| Task-067 | [Task-067](./tasks/Task-067-review-gap-analysis-and-pr.md) | in progress | A |
...
- [x] **TC-379, TC-390:** changed-path allowlist and permitted-path gate (authored first, green last).
- [x] **TC-331..348:** interface, IR byte-identity, diagnostics, determinism, `tsp --emit` parity, namespace scope, locus form, ordering, lint and declaration coverage, pins and licence.
- [x] **TC-349..360:** TypeScript and Rust goldens, purity, missing-base and cycle guards, rename and optional rendering, inventory limitation, unchanged goldens.
- [x] **TC-361..369:** adapter golden, forbidden keys, `$id` and title handling, `RecordString` localisation, purity, unchanged Python evidence, pinned constants, no process spawn.
- [x] **TC-370..382, TC-397:** spike replay, declared delta, lockfile seeding and `--check` failure, dependency removal, frozen-lockfile install, allowlist, changed-path proof, doc and NFR-006 notes.
- [x] **TC-383..389:** repeat-run identity, collator independence, explicit `baseDir`, seeded lockfile, pin inspection, issue #42 record.
- [x] **TC-390..396:** permitted paths, manifest and packed-file surface, licence, dependency sets, restore rehearsal, no publication.
