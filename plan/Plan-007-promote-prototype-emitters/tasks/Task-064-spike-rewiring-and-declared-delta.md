...
- [x] Rewrite `spikes/typespec-feasibility/scripts/run-experiment.mjs` to import the promoted backends and shell `src/compiler/cli.mjs`, stamping `@agent-ix/typespec-semantic-ir-emitter-spike@0.0.0` so the retained IR stays byte-identical.
- [x] Delete `spikes/typespec-feasibility/emitter/`; drop the dependency from both `package.json` files; regenerate `pnpm-lock.yaml` and confirm no `file:`/`link:` specifier survives.
- [x] Seed the committed `Cargo.lock` into each generated Rust package before `cargo check --offline --locked`; make `--check` mode fail rather than generate when the committed lockfile is absent.
- [x] Apply the declared delta: the `command` field of `evidence/custom.json`. Nothing else under `generated/`, `evidence/`, or `report.md` changes.
- [x] Add the `## Retained evidence` note to `docs/semantic-data-system/typespec-feasibility.md` and the one-paragraph NFR-006 amendment (TC-381, TC-397).
- [x] Prove zero publications and mutations with a changed-path check over `git diff origin/main...HEAD`, not by reading `validation.json`'s literals (TC-380).
