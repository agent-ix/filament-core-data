...
- [ ] `src/compiler/inventory.json` with `components` (one record per the fourteen FR-040 Inputs entries: `component`, `source`, `capability`, `disposition`, `targets`, `evidence`, `limitation`) and `authored` (`path`, `reason`).
- [ ] Enforce the closed disposition set, non-empty limitations, target existence, and full `src/compiler/` file ownership, with mutation runs for each rejection path (TC-321, TC-323, TC-326, TC-329, TC-330).
- [ ] Restate the recorded limitation of every `partial` capability the inventory cites (TC-329).
- [ ] Add the `## Promotion inventory` section to `docs/semantic-data-system/typespec-feasibility.md` with counts equal to the inventory's (TC-328).
