...
- [ ] **StR-001:** Keep the semantic data system governed while its compiler becomes real.
- [ ] **US-010:** One command reads a package's sources, manifest, imports, profile, and lock and produces one versioned IR document with its diagnostics.

### Functional Requirements

- [ ] **FR-045:** One frontend seam and dialect registry; `spec-bundle` registered and refused, not guessed.
- [ ] **FR-053:** A closed fifteen-decorator TypeSpec vocabulary and one identity minting rule shared with FR-034.
- [ ] **FR-046:** The lowering from a compiled TypeSpec program to contract IR `1.1.0`, through a confined `CompilerHost`.
- [ ] **FR-047:** Package graph resolution with exact JSON source loci, deterministic candidate selection, and cycle diagnostics.
- [ ] **FR-048:** The RFC 8785 canonical form, the byte sets every digest is defined over, and lock build and verification.
- [ ] **FR-049:** Two registered diagnostic namespaces, deterministic ordering, sort-before-truncate, and declared limits.
- [ ] **FR-050:** Schema validation, a 22-rule cross-field reader, the normalized serialization, and the IR fingerprint.
- [ ] **FR-051:** The compatibility classifier and the two contract-version projections under one published policy.
- [ ] **FR-052:** `compilePackage` and the `compile`, `inspect`, and `diff` verbs behind the narrow interface.

### Non-Functional Requirements

- [ ] **NFR-019:** Byte-identical output across every host-varying input, with every read observed by one injected host.
- [ ] **NFR-020:** Untrusted input, finite limits, no code execution, no network, no path escape, no stray write.
- [ ] **NFR-021:** No publication, no consumer move, no export-surface change, no frozen byte touched, clean restore.
...
|---|---|---|---|
| Task-068 | [Task-068](./tasks/Task-068-guards-and-red-suite.md) | pending | A |
| Task-069 | [Task-069](./tasks/Task-069-host-diagnostics-and-json-loci.md) | pending | A |
| Task-070 | [Task-070](./tasks/Task-070-canonicalization-digests-and-lock.md) | pending | A |
| Task-071 | [Task-071](./tasks/Task-071-package-graph-resolution.md) | pending | A |
| Task-072 | [Task-072](./tasks/Task-072-frontend-seam.md) | pending | A |
| Task-073 | [Task-073](./tasks/Task-073-semantic-vocabulary-and-identities.md) | pending | A |
| Task-074 | [Task-074](./tasks/Task-074-typespec-structural-lowering.md) | pending | A |
| Task-075 | [Task-075](./tasks/Task-075-ir-reader-and-normalization.md) | pending | B |
| Task-076 | [Task-076](./tasks/Task-076-compatibility-diff-and-evolution.md) | pending | B |
| Task-077 | [Task-077](./tasks/Task-077-pipeline-cli-and-interface.md) | pending | A |
| Task-078 | [Task-078](./tasks/Task-078-determinism-safety-and-non-disruption-gates.md) | pending | A |
| Task-079 | [Task-079](./tasks/Task-079-review-gap-analysis-and-pr.md) | pending | A |
...
- [ ] **TC-398..411:** the seam, the dialect registry, the refusal of `spec-bundle`, the shared harness, the never-throw discipline.
- [ ] **TC-412..431:** the fifteen decorators, argument validation, identity minting against FR-034, the constraint alias, the four extensions, declared loss.
- [ ] **TC-432..455:** the structural-kind and scalar tables, roles, multiplicity, nullability, defaults, units, kernel scalars, the envelope, ordering, origins, the confined host, and the frozen-prototype proof.
- [ ] **TC-456..476:** manifest reading and loci, candidate selection, exports and capabilities, cycles, profiles, mappings, targets, confinement, and the graph-case corpus.
- [ ] **TC-477..491:** the RFC 8785 vectors, every digest's byte set, the fingerprint's inclusion and exclusion contract, lock build and verification.
- [ ] **TC-492..509:** the two registered namespaces, code shape, ordering, sort-before-truncate, limits, blocking discipline, and the published registry.
- [ ] **TC-510..526:** schema validation, the 22 cross-field rules, three-reader agreement, normalization, idempotence, the fingerprint, termination.
- [ ] **TC-527..546:** every compatibility case, the family map, the two projections and their goldens, the round trip, the published policy.
- [ ] **TC-547..566:** `emit-ir` unchanged, the five compile phases, the three verbs, exit codes, the fifteen-symbol interface, and the unchanged package manifest.
- [ ] **TC-567..578:** repeat-run and varied-environment byte identity, ambient-input analysis, permutation and collator independence, injected-host observation.
- [ ] **TC-579..589:** the four size limits, path escape, module refusal, no network, the writer, cyclic termination, the fuzz run, message truncation.
- [ ] **TC-590..597:** changed paths, package manifest, frozen paths, the issue #4 goldens, `conformance/`, the restore rehearsal, licences, no publication.
- [ ] **TC-598..619:** the permutation, boundary, error-path, state-transition, and edge-case rows.
