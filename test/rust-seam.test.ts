...
describe("TC-1388..1395 the Rust backend reached through the seam (FR-130)", () => {
	...
	it("renders the Rust backend as the generated crate's generator identity", async () => {
		...
		const identitySource = written.get("src/identity.rs") as string;

		expect(identitySource).toContain(
			`pub const GENERATOR_IDENTITY: &str = "${rustBackend.identity}"`,
		);
		expect(identitySource).not.toContain("backend/typescript");
		console.log(
			`TC-1391 measured: src/identity.rs names ${rustBackend.identity}`,
		);
	});
