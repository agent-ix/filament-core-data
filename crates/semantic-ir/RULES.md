...
| `INVALID_MULTIPLICITY` | `contracts-v1.md` "`multiplicity { lower, upper?, ordered, unique }` (absent `upper` is" | A present `upper` below `lower` fails at `.../multiplicity/upper`. `ordered` and `unique` are required booleans on every multiplicity (QSpec model-complete.md); the schema layer refuses their absence. Their *value* is policed by `FLAGS_ON_NON_COLLECTION` below, not by this code. |
| `FLAGS_ON_NON_COLLECTION` | `contracts-v1.md` "and `true` there is refused with `FLAGS_ON_NON_COLLECTION`, since ordering and" | `multiplicity.ordered` or `multiplicity.unique` is `true` while `upper` is present and at most `1`, at the multiplicity object (FCD #199/#200 review R2: every reader — Rust, Node, TypeScript, Python, semantic-core — refuses this the same way). |
...
| `INVALID_RELATIONSHIP_SOURCE` | `contracts-v1.md` "`sourceEnd` and `targetEnd` name one in `type`" | A relationship `sourceEnd.type` naming a type other than the one declaring the relationship (FCD #199/#200 review finding 9); the source end always names its own declaring type, so this is checked directly rather than assumed. |
...
| `UNRESOLVED_FRAME_PATH` | `contracts-v1.md` "Declaration references the operation `modifies` (a field or relationship), `creates` or `deletes` (an object type or process); never an access path" | `2.0.0` only: a `frame` `modifies` entry naming no field or relationship the document declares, or a `creates`/`deletes` entry naming no type the document declares. Resolution ranges over the whole document, never only the owning type or its supertypes (QSpec FR-340, FR-013). |
