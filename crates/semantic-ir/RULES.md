...
| `INVALID_MULTIPLICITY` | `contracts-v1.md` "`multiplicity { lower, upper?, ordered, unique }` (absent `upper` is" | A present `upper` below `lower` fails at `.../multiplicity/upper`. `ordered` and `unique` are required booleans on every multiplicity (QSpec model-complete.md); the schema layer refuses their absence. Their *value* is never policed by any code — every producer clamps both to `false` when `upper` is at most `1` (owner ruling 2026-09-19T15:39:32Z on FCD #199); `FLAGS_ON_NON_COLLECTION`, which R2 of the #199/#200 review round had readers refuse this on, is deleted, not corrected. |
