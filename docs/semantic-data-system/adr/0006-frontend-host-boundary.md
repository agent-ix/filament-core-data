...
Recorded here rather than quietly amended, because the falsification is the
useful part.

Adding a process capability to `src/compiler/host.mjs` fails
**NFR-020-AC-5**, which is not a changed-path gate or an annexation artifact but
a normative safety property with a test behind it:
...
The scope is every module under `frontend/`, `packages/`, `ir/`, `compat/`, plus
`pipeline.mjs`, `cli.mjs`, `json-locus.mjs` and `host.mjs`. The forbidden set is
`node:net`, `node:http`, `node:https`, `node:dgram`, `node:child_process`,
`node:worker_threads` and `node:vm`. `host.mjs` is named explicitly, and so is
`cli.mjs` — so the capability cannot be moved to the caller inside this
repository's compiler either.

Six suites went red, and every one of them was right.

## Decision

**Deferred, pending an owner ruling.** The two candidates below are not
equivalent, and choosing between them changes a published acceptance criterion or
a published seam contract. Recording the analysis without recording a decision is
the honest state.

### Option A — the compiler never spawns

The lift runs outside `src/compiler/`, and the already-lifted document reaches
the seam as data.

Preserves NFR-020-AC-5 exactly as written. The cost is that frontend selection
moves to the call site: a caller that must lift before it can select has already
selected, which is the property FR-045's seam exists to prevent. The seam would
need a stated way to accept a pre-lifted document, which is a change to its
contract rather than to its implementation.

### Option B — the capability is caller-declared, and the criterion says so

`host.mjs` gains the bounded process capability, and NFR-020-AC-5 is narrowed
from "imports no code-executing built-in" to "executes nothing derived from an
input; the one module that may execute a caller-declared program is the host,
and every execution is recorded".

The argument for it is that NFR-020's threat model is **input-controlled
execution**. That is what `getJsImport`'s refusal and `moduleRoots` defend
against: a *package* must not be able to make the compiler run code. A program
the caller declares by name is not package-controlled — no bundle can name it,
and no input can introduce one. The blanket module-import gate cannot distinguish
caller-declared from input-derived execution, which is why it fires here, and
that is a limitation of the check rather than a violation of the property.

The cost is real: "the compiler executes nothing" is a stronger and cheaper claim
to verify than "the compiler executes only what its caller declared", and the
weaker claim puts more weight on the host's record being read.

### What is decided either way

Whichever is chosen, two things hold and do not need re-deciding:

- The frontend names no absolute path and opens no file directly. It reaches the
  producer and its artifacts through the injected host.
- Every invocation is recorded with the executable's resolved path, its digest
  and its arguments, so "which binary produced this document" is answerable from
  the record rather than from the reader's memory of what was on the path.

## Superseded draft

The original decision, kept for the record: the `spec-bundle` frontend reaches
the Rust extraction binary **through the injected host**, and the host gains a
bounded process capability to make that possible. No module under `src/compiler/frontend/` imports `node:fs`,
...
**Proposed, not normative.** It does not govern the #86 wiring until the owner
rules between Option A and Option B, because both options change something
published — a safety criterion in one case, a seam contract in the other.

What it does govern now is that the wiring is not written until that ruling
exists. The first draft of this decision was implemented and failed six suites;
shipping it would have meant deleting a safety gate to make a convenience pass,
at the moment its author had the least context for that trade. That is the
failure #100 predicted when it said to write this ADR before the plumbing.
