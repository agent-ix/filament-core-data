...
def _source_snapshot() -> dict[str, bytes]:
    """Snapshot owned source and generated bytes, excluding interpreter caches."""
    return {
        str(path.relative_to(REPO)): path.read_bytes()
        for directory in (BACKEND, REPO / "schema" / "semantic" / "v1")
        for path in sorted(directory.rglob("*"))
        if path.is_file() and "__pycache__" not in path.parts
    }


def _interrupted_scratch_check(
    scratch: pathlib.Path, module: str, relative_target: pathlib.Path
) -> None:
    """Run a real copied checker before and after a killed scratch-only writer."""
    before = _source_snapshot()
    shutil.copytree(
        BACKEND,
        scratch / "python_backend",
        ignore=shutil.ignore_patterns("__pycache__"),
    )
    shutil.copytree(
        REPO / "schema" / "semantic" / "v1", scratch / "schema" / "semantic" / "v1"
    )
    env = {**os.environ, "PYTHONPATH": str(scratch), "PYTHONDONTWRITEBYTECODE": "1"}
    ...
    target = scratch / relative_target
    ...
    assert "differs from a fresh" in failed.stderr
    assert _source_snapshot() == before
...
def test_a_mutated_committed_report_fails_check(tmp_path: pathlib.Path) -> None:
    ...
    _interrupted_scratch_check(
        tmp_path,
        "python_backend.runner.qualify",
        pathlib.Path("python_backend/qualification/report.json"),
    )
...
def test_a_mutated_generated_file_fails_check(tmp_path: pathlib.Path) -> None:
    """TC-921: FR-079-AC-4 (the falsification half)."""
    _interrupted_scratch_check(
        tmp_path,
        "python_backend.runner.emit",
        pathlib.Path("python_backend/generated")
        / sorted(emit.demonstrated())[0]
        / "README.md",
    )
